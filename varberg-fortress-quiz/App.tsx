...
import { QUIZ_DATA, IMAGE_URL } from './constants';

type Feedback = {
  type: 'correct' | 'incorrect' | 'finished';
  message: string;
};

// Helper component for individual clickable points on the image
interface ClickablePointProps {
  item: QuizItem;
...
const ClickablePoint: React.FC<ClickablePointProps> = ({ item, onClick, isGuessed, isDisabled }) => {
...
const App: React.FC = () => {
  ...
  const [mistakes, setMistakes] = useState<number>(0); // Track incorrect clicks
  ...
  const handlePointClick = useCallback((clickedId: number) => {
